...
&& cd slackin-src && npm install --unsafe-perm

COPY shellscripts/parse_variables.sh /slackin/parse_variables.sh

EXPOSE 8000

ENV SLACK_CHANNELS SLACK_SUBDOMAIN SLACK_API_TOKEN GOOGLE_CAPTCHA_SECRET GOOGLE_CAPTCHA_SITEKEY

CMD ["/bin/sh", "-c", "/slackin/slackin-src/bin/slackin --hostname '0.0.0.0' --port '8000' --channels $(echo $(sh /slackin/parse_variables.sh))"]
